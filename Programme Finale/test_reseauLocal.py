...
def main():
    ...
    while True:
        ...
        if refresh != -1:
            ...
            for sensor_config in config['capteurs']:
                ...
                if sensor['type'] == 'BME280' and sensor['active']:
                    ...
                    vitesse_vent, average_direction = sensor_anemometre.measure_anemometer_data(sensor['pin_digital'], None, wind_readings)
                    sensor['values'] = vitesse_vent
                    data_json['capteurs'].append(sensor)
                    #sensor_anemometre.write_anemometer_values(config, vitesse_vent)
                elif sensor['type'] == 'WINDVANE' and sensor['active']:
                    vitesse_vent, average_direction = sensor_anemometre.measure_anemometer_data(None, sensor['pin_analog'], wind_readings)
                    sensor['values'] = average_direction
                    data_json['capteurs'].append(sensor)
                    #sensor_anemometre.write_windVane_values(config, average_direction)
                    
            #write_json_file('config_sondes.json', config)
        ...
        #sensors_data = read_json_file('config_sondes.json')
        json_data = json.dumps(data_json)
        conn.send('HTTP/1.1 200 OK\n')
        conn.send('Connection: close\n')
        conn.send('Content-Type: application/json\n')
